...
(function() {
    ...
    function extractContent(doc, query) {
        ...
        // 标题
        const wordTitle = query;
        content += `<div style="border-bottom:2px solid #2575fc; padding-bottom:10px; margin-bottom:15px;">
            <h2 style="color:#2575fc;"><i class="fas fa-book"></i> ${escapeHtml(wordTitle)}</h2>
        </div>`;
        ...
        for (let i = 0; i < expList.length; i++) {
            ...
            const meaning = expList[i]?.textContent.trim() || '';
            content += `<div style="margin-bottom:12px; padding:10px; background:#f8f9fa; border-radius:6px; border-left:4px solid #2575fc;">
                ${pos ? `<em style="color:#6c757d;">${escapeHtml(pos)}</em> ` : ''}
                <strong>${escapeHtml(meaning)}</strong>
            </div>`;

            // 对应例句
            const example = egList[i]?.textContent.trim();
            if (example) {
                content += `<div style="margin-left:20px; margin-bottom:8px; color:#495057;">
                    ${escapeHtml(example)}
                </div>`;
            }
        }
